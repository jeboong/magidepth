...
4. Publish `vX.Y.Z` with `MagiMagic-Setup-X.Y.Z.exe`, `.blockmap`, `latest.yml`, and `SHA256SUMS.txt`. Keep v0.1's MagiDepth assets unchanged.
...
The application name is MagiMagic from 0.2 onward; `com.magidepth.app`, npm package identity, GitHub repository, update provider and `%APPDATA%/MagiDepth` remain stable. Changing these casually would create duplicate caches or break the upgrade path. Do not close a user's running renderer to install an update without checking its state/obtaining direction.
