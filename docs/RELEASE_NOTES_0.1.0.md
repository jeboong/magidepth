...
RTX 5070 Ti에서 두 Depth 모델, 고급 8종 PNG 및 짧은 MP4, HEVC, 트림, 취소를 실제 검증했습니다. 별도로 격리 Python 런타임과 Electron IPC·클립보드·설정·소스 출력 통합 테스트를 수행했습니다. NSIS 설치가 정상 완료되었고, 실제 설치된 ASAR와 리소스에서 공개 업데이트 피드 확인을 포함한 11개 통합 검사를 통과했습니다. 현재 버전의 최신 상태 확인이며, 향후 버전으로의 실제 업그레이드 완료를 검증한 것은 아닙니다. [실측 조건](https://github.com/jeboong/magidepth/blob/v0.1.0/backend/BENCHMARKS.md)
