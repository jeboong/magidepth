...
import {requireLocalPath,safeAssetPath,sanitizeOptions,sanitizePreferences} from './policy';
...
async function savePrefs(update:any){
  prefs=sanitizePreferences({...prefs,...update,options:{...prefs.options,...update?.options}});
...
async function engine(){
  if(!(await runtime.inspect()).ready)throw new Error('먼저 AI 엔진 설치를 완료해 주세요.');
  worker??=new PythonWorker({python:runtime.pythonPath,backend,bin:runtime.binDir,models:runtime.modelsDir,logs:path.join(app.getPath('userData'),'logs')},event=>send('depth:progress',event));
  return worker;
}
...
async function sourcePath(value:unknown){
  ...
  if(!/\.(mp4|mov|mkv|avi|webm|m4v|mts|mxf|png|jpg|jpeg|webp|bmp|tif|tiff)$/i.test(file))throw new Error('지원되는 이미지 또는 영상 파일을 선택해 주세요.');
...
function setupIPC(){
  ...
  handle('runtime:get',()=>runtime.inspect());handle('runtime:install',()=>runtime.install());
  handle('system:get',async()=>({...await(await engine()).request<any>(crypto.randomUUID(),'system',{}),appVersion:app.getVersion()}));
...
async function createWindow(){
  ...
  window=new BrowserWindow({width:1440,height:960,minWidth:1040,minHeight:720,backgroundColor:'#101113',title:'MagiDepth · 매지댑스',autoHideMenuBar:true,show:false,icon:path.join(root,'dist','brand','magidepth.png'),webPreferences:{preload:path.join(__dirname,'preload.cjs'),nodeIntegration:false,contextIsolation:true,sandbox:true,webSecurity:true}});
...
if(!lock)app.quit();else{
  ...
  app.whenReady().then(createWindow).catch(err=>{dialog.showErrorBox('MagiDepth 시작 실패',String(err));app.quit();});
  app.on('window-all-closed',()=>app.quit());
  app.on('before-quit',()=>{worker?.stop();runtime?.stop();});
}
