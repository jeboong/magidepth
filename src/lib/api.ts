...
const demo: DepthDeskAPI = {
  ...
  cancelJob: async () => {},
  preparePlayback: async ({path}) => ({path, proxy: false, cached: true}),
  cancelPlayback: async () => {},
  onPlaybackProgress: () => () => {},
  getModelCatalog: async () => ({basicReady: false, models: downloadModelIds.map(id => ({id, name: id, ready: false,
    builtin: id === 'image-small' || id === 'video-small', repo: '', revision: '', license: '', reason: '모델 다운로드는 데스크톱 앱에서 사용할 수 있습니다.'}))}),
  downloadModel: async () => {throw new Error('모델 다운로드는 데스크톱 앱에서 실행됩니다.');},
